  ...
  return (
    <div className="p-6 md:p-10 max-w-5xl">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-8">
        <p className="text-xs font-ui tracking-[0.2em] uppercase text-gold mb-2">Curated Selection</p>
        <h1 className="font-display text-4xl md:text-5xl text-cream font-bold mb-2">Discover</h1>
        <p className="font-serif-body text-muted-foreground">The world's finest cigars, curated for your palate</p>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="mb-6 space-y-4">
        <div className="relative">
          ...
          <input
            ...
            placeholder="Search by name, brand, origin, or flavor..."
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {filtered.map((cigar, i) => (
          <motion.button
            ...
            className="gradient-card border border-border hover:border-gold/40 p-5 text-left transition-all duration-300 hover:shadow-card group"
          >
            ...
            <div className="flex gap-1.5 flex-wrap mt-3">
              {cigar.tags.slice(0, 3).map((tag) => (
                <span key={tag} className="text-xs font-ui px-2 py-0.5 bg-muted text-muted-foreground border border-border">
                  {tag}
                </span>
              ))}
            </div>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-16 text-muted-foreground">
          <p className="font-display text-xl text-cream mb-2">No cigars found</p>
          <p className="font-serif-body italic">Try a different search or filter</p>
        </div>
      )}
      ...
      {selected && (
        ...
        >
          ...
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
            className="relative bg-card border border-border shadow-deep w-full max-w-md"
          >
            <div className="p-6">
              <div className="flex justify-between items-start mb-2">
                <p className="text-xs font-ui text-gold tracking-wider uppercase">{selected.brand}</p>
                <button onClick={() => setSelected(null)} className="text-muted-foreground hover:text-cream text-xs font-ui tracking-wider">Close ✕</button>
              </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-4 text-sm">
                {[["Origin", selected.origin], ["Wrapper", selected.wrapper], ["Size", selected.size]].map(([l, v]) => (
                  <div key={l}>
                    <p className="text-xs font-ui text-muted-foreground uppercase tracking-wider mb-0.5">{l}</p>
                    <p className="font-ui text-foreground">{v}</p>
                  </div>
                ))}
              </div>
