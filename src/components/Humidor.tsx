    ...
    <div className="p-6 md:p-10 max-w-5xl">
      ...
      >
        <div className="flex items-start justify-between">
          <div>
            <p className="text-xs font-ui tracking-[0.2em] uppercase text-gold mb-2">My Collection</p>
            <h1 className="font-display text-4xl md:text-5xl text-cream font-bold mb-2">Humidor</h1>
            <p className="font-serif-body text-muted-foreground">
              {cigars.length === 0 ? "Your sanctuary awaits. Add your first cigar." : `${cigars.length} fine cigar${cigars.length === 1 ? "" : "s"} in your collection`}
            </p>
          </div>
          <button
      ...
      {cigars.length === 0 && !showForm && (
        ...
        >
          ...
          <p className="font-display text-2xl text-cream mb-3">Your humidor is empty</p>
          <p className="font-serif-body text-muted-foreground mb-8 max-w-sm mx-auto">
            Every great collection begins with a single exceptional cigar. Add yours.
          </p>
          <button
            onClick={() => setShowForm(true)}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
          {cigars.map((cigar, i) => (
            <motion.div
              ...
              className="gradient-card border border-border hover:border-gold/40 p-5 cursor-pointer group transition-all duration-300 hover:shadow-card"
            >
              ...
              <div className="space-y-1.5 mb-3">
                <div className="flex justify-between text-xs">
                  <span className="text-muted-foreground font-ui">Origin</span>
                  <span className="text-foreground font-ui">{cigar.origin}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-muted-foreground font-ui">Wrapper</span>
                  <span className="text-foreground font-ui truncate ml-2 text-right">{cigar.wrapper}</span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-muted-foreground font-ui">Strength</span>
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-center mb-6">
                  <h2 className="font-display text-2xl text-cream">Add to Humidor</h2>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-1.5 block">Origin</label>
                    ...
                    <div>
                      <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-1.5 block">Strength</label>
                      <div className="relative">
                        <select
                          value={form.strength}
                  ...
                  <div>
                    <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-1.5 block">Wrapper</label>
                    <div className="relative">
                      <select
                        value={form.wrapper}
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-1.5 block">Size / Vitola</label>
                      <input
                        value={form.size}
                        onChange={(e) => setForm({ ...form, size: e.target.value })}
                        placeholder="e.g. Robusto, 5×52"
                        className="w-full bg-input border border-border px-3 py-2.5 text-sm text-foreground font-ui placeholder:text-muted-foreground/50 focus:outline-none focus:border-gold/50 transition-colors"
                      />
                    </div>
                    <div>
                      <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-1.5 block">Quantity</label>
                      <input
                        type="number"
                        min={1}
                        value={form.quantity}
                  ...
                  <div>
                    <label className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-2 block">Your Rating</label>
            ...
            >
              <div className="p-6">
                <div className="flex justify-between items-start mb-4">
                  ...
                  <button onClick={() => setSelectedCigar(null)} className="text-muted-foreground hover:text-cream">
                    <X size={18} />
                  </button>
                </div>
                ...
                <div className="space-y-2 mb-4">
                  {[
                    ...
                    ["Size", selectedCigar.size || "Not specified"],
                    ["Quantity", selectedCigar.quantity.toString()],
                    ["Added", selectedCigar.addedDate],
                  ].map(([label, value]) => (
                ...
                {selectedCigar.notes && (
                  <div className="border-t border-border pt-4">
                    <p className="text-xs font-ui tracking-wider uppercase text-muted-foreground mb-2">Tasting Notes</p>
