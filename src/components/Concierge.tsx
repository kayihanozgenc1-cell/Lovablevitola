...
const levelContext = {
  novice: "beginner who is just starting to explore cigars",
  aficionado: "cigar aficionado with good knowledge of blends, origins, and brands",
  collector: "serious cigar collector with deep expertise in rare, vintage, and premium cigars",
};
...
export function Concierge({ level }: ConciergeProps) {
  const [messages, setMessages] = useState<Message[]>([
    {
      role: "assistant",
      content: `Good evening. I am your personal cigar concierge — here to guide you through the world of fine tobacco. As a ${levelContext[level]}, I'll tailor my guidance to your expertise. How may I assist you today?`,
    },
  ]);
  ...
  const handleSend = async (text?: string) => {
    ...
    // Simulate AI response (in a real app, connect to Lovable Cloud AI)
    ...
    const reply =
      ...
      `An excellent question for a ${levelContext[level]}. The world of premium cigars is vast and rewarding. Each cigar tells the story of its terroir, the master blender's vision, and the artisan's craft. I'd be delighted to guide you further — could you share more about what you're looking to explore or experience?`;
    ...
    <div className="flex flex-col h-screen md:h-auto md:min-h-0" style={{ height: "calc(100vh - 0px)" }}>
      ...
      <div className="p-6 md:p-10 pb-4 md:pb-6 flex-shrink-0">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}>
          <p className="text-xs font-ui tracking-[0.2em] uppercase text-gold mb-2">AI Powered</p>
          <h1 className="font-display text-4xl md:text-5xl text-cream font-bold mb-2">Cigar Concierge</h1>
          <p className="font-serif-body text-muted-foreground">Your personal cigar expert, available day and night</p>
      ...
      <div className="flex-1 overflow-y-auto px-6 md:px-10 pb-4 space-y-4 min-h-0">
        ...
        {loading && (
          ...
          >
            <div className="w-7 h-7 flex items-center justify-center border border-gold/40 bg-gold/10 text-gold">
              <Bot size={13} />
            </div>
      ...
      <div className="px-6 md:px-10 py-4 border-t border-border flex-shrink-0">
        <div className="flex gap-3">
          <input
            ...
            placeholder="Ask about cigars, pairings, aging, origins..."
